...
// пустой массив куда будемKa	 добавлять тип поля, названия поля и подписи к поле
var arrayInfoFields = [];
	...
	for (var i = 0; i < array.length; i++) {
		...
		switch(array[i].type) {
			...
			case "radio":
				...
				keys.forEach(function(val, ind, arr) {
					// if (ind === (arr.length-1)) {
					// 	var temporary = "<input type='" + array[i].type + "' name='" + array[i].name + "' value='" + (ind + 1) + "' checked>" + arr[ind];
					// } else {
